...
class AniListApi : SyncAPI() {
    ...
    override suspend fun login(redirectUrl: String, payload: String?): AuthToken? {
        ...
        return AuthToken(
            ...
            accessTokenLifetime = APIHolder.unixTime + response.expiresIn,
        )
    }
    ...
    data class TokenResponse(
        ...
        @JsonProperty("expires_in") @SerialName("expires_in") val expiresIn: Long,
    )

    override suspend fun refreshToken(token: AuthToken): AuthToken? {
        // AniList access tokens are long-lived.
        // They will remain valid for 1 year from the time they are issued.
